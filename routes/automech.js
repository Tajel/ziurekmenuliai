...
router.get("/", middleware.isLoggedIn, function(req, res){
    ...
    Automech.find({doc:"automech"}, function(err, alltrmech){
       if(err){
           console.log(err);
       } else {
          ...
          res.render("automech/index",{alltrmech:alltrmech});
       }
    });
});
...
router.post("/", middleware.isLoggedIn, function(req, res){
    ...
    Autokellap.findOne({doc:'automech'}, function(err, result){
        if(err){
            console.log(err);
        }if(!result){
            Autokellap.create({doc:'automech'}, function(err, auto){
                if(!err){
                    Automech.create(automech, function(err, newlyCreated){
                        if(err){
                            console.log(err);
...
router.get("/new", middleware.isLoggedIn, function(req, res){
    Automech.find({$or:[{ padkodas: { $exists: true } },{ trrus: { $exists: true } },{ trtipas: { $exists: true } },{ vairVardPav: { $exists: true } },{ kurrusis: { $exists: true } }]}).exec(function (err, dataSum) {
        if(!err){
            // console.log("rasti Automech: " + dataSum);
            res.render("automech/new", {data:dataSum}); 
            // console.log("rasti Automech: " + dataSum);
        }
    });
});
...
router.get("/:id/krovkellap", function(req, res){
    ...
    Automech.findById(req.params.id).populate("kellapas").exec(function(err, foundtransport){
        if(err){
            ...
            res.render("automech/showkellap", {automech: foundtransport});
        }
    });
});
...
router.get("/:id", function(req, res){
    ...
    Automech.findById(req.params.id).exec(function(err, foundtransport){
        if(err){
            ...
            res.render("automech/show", {automech: foundtransport});
        }
    });
});
    ...
    Automech.findById(req.params.id, function(err, foundtransport){
        if(!err){
            ...
            Automech.find({$or:[{ padkodas: { $exists: true } },{ trrus: { $exists: true } },{ trtipas: { $exists: true } },{ vairVardPav: { $exists: true } },{ kurrusis: { $exists: true } }]}).exec(function (err, dataSum) {
                if(!err){
                    console.log("rasti Automech: " + dataSum);
                    res.render("automech/edit", {automech: foundtransport, data:dataSum});
                }
            });
        }
    });
