...
router.get("/", middleware.isLoggedIn, function(req, res){
    ...
    Obj.find({doc:"objektas"}, function(err, allobjekt){
       if(err){
           console.log(err);
       } else {
           ...
           res.render("obj/index",{allobjekt:allobjekt});
       }
    });
});
...
router.post("/", middleware.isLoggedIn, function(req, res){
    ...
    var newObj = {doc: doc, dvVadasPav: dvVadasPav, objkodas: objkodas, objpav:objpav}
...
router.get("/new", middleware.isLoggedIn, function(req, res){
    Obj.find({doc:"darbvad"},function(err, allDv){
        if(!err){
...
router.get("/:id/edit", middleware.isLoggedIn, function(req, res){        //  middleware.checkOwnership
    Obj.findById(req.params.id, function(err, foundObj){
        res.render("obj/edit", {obj: foundObj});
    });
});
