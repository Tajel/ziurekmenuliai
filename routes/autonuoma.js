...
router.get("/", middleware.isLoggedIn, function(req, res){
    ...
    Autonuoma.find({doc:"autonuoma"}, function(err, alltrnuom){
       if(err){
           console.log(err);
       } else {
          ...
          res.render("autonuoma/index",{alltrnuom:alltrnuom});
       }
    });
});
...
router.get("/new", middleware.isLoggedIn, function(req, res){
    Autonuoma.find({$or:[{ padkodas: { $exists: true } },{ trrus: { $exists: true } },{ kurrusis: { $exists: true } }]}).exec(function (err, dataSum) {
        if(!err){
...
router.get("/:id/krovkellap", function(req, res){
    ...
    Autonuoma.findById(req.params.id).populate("kellapas").exec(function(err, foundtransport){
        if(err){
            ...
            res.render("autonuoma/showkellap", {autonuoma: foundtransport});
        }
    });
});
...
router.get("/:id", function(req, res){
    ...
    Autonuoma.findById(req.params.id).exec(function(err, foundtransport){
        if(err){
            ...
            res.render("autonuoma/show", {autonuoma: foundtransport});
        }
    });
});
    ...
    Autonuoma.findById(req.params.id, function(err, foundtransport){
        if(!err){
            ...
            Autonuoma.find({$or:[{ padkodas: { $exists: true } },{ trrus: { $exists: true } },{ kurrusis: { $exists: true } }]}).exec(function (err, dataSum) {
                if(!err){
                    // console.log("rasti Autonuoma: " + dataSum);
                    res.render("autonuoma/edit", {autonuoma: foundtransport, data:dataSum});
                }
            });
        }
    });
