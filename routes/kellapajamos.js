    ...
    if (err) {
      ...
      Kellapajamos.create(req.body.kellap, function(err, kellapdata) {
        if (err) {
          ...
          req.flash(
            "success",
            "Suvestas objektas!" + req.body.kellap["iobjkodas"]
          );
  ...
  async.series(
    ...
    function(err, response) {
      if (err) {
        ...
        console.log("=================================");
        // console.log("=============response==============" + response)
        res.render("kellapajamos/edit", {
          data: response,
          kellapId: req.params.id,
          pajamosId: req.params.pajamosId
        });
      }
    }
  );
...
router.put("/:pajamosId", function(req, res) {
  Kellapajamos.findByIdAndUpdate(
    req.params.pajamosId,
    req.body.kellap,
    function(err, kellap) {
      if (err) {
        res.render("edit");
