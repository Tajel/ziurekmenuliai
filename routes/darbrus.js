...
router.get("/", middleware.isLoggedIn, function(req, res){
    ...
    Darbrus.find({doc:"darbrus"}, function(err, allDarbrus){
       if(err){
           console.log(err);
       } else {
           ...
           res.render("darbrus/index",{alldarbrus:allDarbrus});
       }
    });
});
...
router.post("/", middleware.isLoggedIn, function(req, res){
    ...
    var newDarbrus = {doc: doc, darbrus: darbrus, vnt:vnt,author:author}
...
router.get("/:id/edit", middleware.isLoggedIn, function(req, res){        //  middleware.checkOwnership
    Darbrus.findById(req.params.id, function(err, foundDarbrus){
        res.render("darbrus/edit", {darbrus: foundDarbrus});
    });
});
