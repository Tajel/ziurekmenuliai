...
router.get("/", middleware.isLoggedIn, function(req, res){
    ...
    Autolen.find({doc:"autolen"}, function(err, alltrlen){
       if(err){
           console.log(err);
       } else {
          ...
          res.render("autolen/index",{alltrlen:alltrlen});
       }
    });
});
...
router.post("/", middleware.isLoggedIn, function(req, res){
    ...
    var autolen = req.body.autolen
        autolen.author = author;
    // Create a new Autolen and save to DB
    Autokellap.findOne({doc:'autolen'}, function(err, result){
        if(err){
            console.log(err);
        }if(!result){
            Autokellap.create({doc:'autolen'}, function(err, auto){
                if(!err){
                    Autolen.create(autolen, function(err, newlyCreated){
                        if(err){
                            console.log(err);
...
router.get("/new", middleware.isLoggedIn, function(req, res){
    Autolen.find({$or:[{ padkodas: { $exists: true } },{ trrus: { $exists: true } },{ trtipas: { $exists: true } },{ vairVardPav: { $exists: true } },{ kurrusis: { $exists: true } }]}).exec(function (err, dataSum) {
        if(!err){
            // console.log("rasti Autolen: " + dataSum);
            res.render("autolen/new", {data:dataSum}); 
            // console.log("rasti Autolen: " + dataSum);
        }
    });
});
...
router.get("/:id/krovkellap", function(req, res){
    ...
    Autolen.findById(req.params.id).populate("kellapas").exec(function(err, foundtrlen){
        if(err){
            ...
            res.render("autolen/showkellap", {autolen: foundtrlen});
        }
    });
});
    ...
    Autolen.findById(req.params.id, function(err, foundtransport){
        if(!err){
            ...
            Autolen.find({$or:[{ padkodas: { $exists: true } },{ trrus: { $exists: true } },{ trtipas: { $exists: true } },{ vairVardPav: { $exists: true } },{ kurrusis: { $exists: true } }]}).exec(function (err, dataSum) {
                if(!err){
                    // console.log("rasti Autolen: " + dataSum);
                    res.render("autolen/edit", {autolen: foundtransport, data:dataSum});
                }
            });
        }
    });
