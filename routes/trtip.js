...
router.get("/", middleware.isLoggedIn, function(req, res){
    ...
    Trtip.find({doc:"trantipas"}, function(err, trtip){
       if(err){
           console.log(err);
       } else {
           if(req.xhr){
            ...
            //   console.log(alltrtip);
               trtip.sort(function(a, b) {
                    ...
                    // console.log(alltrtip);
               res.render("trtip/index",{trtip:trtip});
           }
        }
    });
});
...
router.post("/", middleware.isLoggedIn, function(req, res){
    ...
    var newtrtip = {doc: doc, trtipas: trtipas}
...
router.get("/:id/edit", middleware.isLoggedIn, function(req, res){        //  middleware.checkOwnership
    Trtip.findById(req.params.id, function(err, trtip){
        res.render("trtip/edit", {trtip: trtip});
    });
});
...
router.put("/:id",middleware.isLoggedIn, function(req, res){
    ...
    Trtip.findByIdAndUpdate(req.params.id, req.body.trtip, {new:true}, function(err, updatedtrtip){
       if(err){
            res.redirect("/trtip");
        } else {
